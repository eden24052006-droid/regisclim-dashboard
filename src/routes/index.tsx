...
function Index() {
  return (
    <>
      ...
      <section className="bg-muted"><div className="content-shell"><div className="max-w-2xl"><p className="eyebrow text-accent">Nos engagements</p><h2 className="mt-3 font-display text-4xl font-extrabold">Du conseil juste, du matériel fiable.</h2></div><div className="mt-10 grid gap-px bg-border md:grid-cols-3">{[[BadgeCheck,"Qualité","Des équipements choisis pour leur fiabilité et leur adéquation à vos besoins."],[ShieldCheck,"Efficacité","Le respect des normes et une solution dimensionnée pour votre lieu de vie ou de travail."],[Leaf,"Économie & écologie","Des appareils nouvelle génération, de classe énergétique A+, A++ ou A+++."]].map(([Icon,title,text]) => { const I=Icon; return <article key={title as string} className="bg-background p-8"><I className="size-8 text-primary" /><h3 className="mt-6 font-display text-xl font-extrabold">{title as string}</h3><p className="mt-3 text-sm leading-7 text-muted-foreground">{text as string}</p></article>})}</div></div></section>

      <section className="content-shell grid items-center gap-12 lg:grid-cols-2"><div className="overflow-hidden"><img src={technicianImage} loading="lazy" width={1200} height={900} alt="Technicien entretenant une climatisation" className="aspect-[4/3] size-full object-cover" /></div><div><p className="eyebrow text-primary">Savoir-faire complet</p><h2 className="mt-3 font-display text-4xl font-extrabold">Installation, entretien et solutions thermiques.</h2><div className="mt-8 grid gap-5">{[[Wrench,"Installation","Mono-split ou multi-split pour obtenir la température souhaitée et un air purifié."],[Gauge,"Entretien","Un suivi régulier évite la surconsommation et l’usure prématurée."],[Home,"Gainable","Une unité discrète dans les combles distribue l’air par un réseau de gaines."],[Leaf,"Chauffe-eau thermodynamique","Un ballon associé à une pompe à chaleur pour réduire la consommation énergétique."]].map(([Icon,title,text])=>{const I=Icon; return <div key={title as string} className="flex gap-4"><I className="mt-1 size-6 shrink-0 text-accent"/><div><h3 className="font-display text-lg font-extrabold">{title as string}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{text as string}</p></div></div>})}</div><Button asChild variant="hero" size="hero" className="mt-8"><a href="/installation-de-climatisation-a-nimes">Voir les installations <ArrowRight /></a></Button></div></section>
      <ContactBand />
    </>
  );
}
